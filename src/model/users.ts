...
class User extends BaseEntity {
    ...
    @Column('varchar', { unique: true, comment: '아이디' }) //일반적인 id
    userName!: string;

    @Column('varchar', { comment: '비밀번호' })
    password!: string;

    @Column('varchar', { comment: 'salt' })
    salt!: string;
    ...
    @Column('varchar', { comment: '성' })
    lastName!: string;

    @Column('varchar', { length: 12, unique: true, comment: '닉네임' })
    nickName!: string;
    ...
    @Column('varchar', { default: '', comment: '자기소개' })
    introduction!: string;
