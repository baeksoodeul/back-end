...
export const getReportList = async (): Promise<Report[] | undefined> => {
    try {
        const reportList = await Report.createQueryBuilder('report')
            .select([
                'report.r_id',
                'report.user',
                'report.category',
...
export const getReportDetail = async (data: number): Promise<Report | undefined> => {
    ...
    try {
        const reportDetail = await Report.createQueryBuilder('report')
            .select(['report'])
...
export const createReport = async (data: newReport): Promise<InsertResult | undefined> => {
    ...
    try {
        ...
        const iReport: InsertResult = await Report.createQueryBuilder()
            .insert()
            .into(Report)
            .values(val)
            .execute();
...
export const updateReport = async (data: number): Promise<UpdateResult | undefined> => {
    ...
    try {
        const uReport: UpdateResult = await Report.createQueryBuilder()
            .update(Report)
            .set({
                infliction: true,
                managedDate: udate
